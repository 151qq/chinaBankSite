...
const tools = {
  ...
  getGameUser (cb) {
    var location = window.location
    var parsed = queryString.parse(location.search)
    var gameUser = store.state.gameUser

    var nextPage = window.encodeURIComponent(window.location.href)

    console.log(gameUser, 'gameUser')

    if (!gameUser.openId) {
      var path = location.origin + '/gameRegistor' + location.search + '&scope=snsapi_userinfo&redirectUrl=' + nextPage
